...
describe('PanamaFuture', function() {
  beforeEach(async function() {
    ...
    this.contract = await PanamaFuture.new(
  ...
  context('Minting Functions', function() {
    it('mint tokens', async function() {
      ...
      expect((await this.contract.balanceOf(user))
          .toString()).to.equal(futuresBought.toString());
      // Check if ecobux was taken from account
      expect((await EcoBuxInstance.balanceOf(user))
          .toString()).to.equal((ecoMint-futuresBought*futurePrice).toString());
    });
    ...
    it('should fail to mint when not enough ecobux', async function() {
      // Mint tokens
      await expectRevert(
          this.contract.buyFuture(1000, {from: accounts[1]}),
          'Not Enough EcoBux',
      );
    });
  });
