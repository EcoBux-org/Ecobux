...
describe("EcoBux", function () {
  ...
  context("Basic ERC20 Functions", function () {
    ...
    it("has decimals", async function () {
      await expect((await this.contract.decimals()).toString()).to.equal("2");
    });
  });
  context("Minting Functions", function () {
    it("mint tokens", async function () {
      ...
      await expect((await this.contract.balanceOf(admin)).toString()).to.equal("1000");
    });
    it("verify only owners can mint tokens", async function () {
      ...
      await expect((await this.contract.balanceOf(admin)).toString()).to.equal("0");
    });
  });
});
